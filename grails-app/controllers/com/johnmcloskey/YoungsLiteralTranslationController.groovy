...
class YoungsLiteralTranslationController {
	...
	//readChapter
	def readChapter() {
		if(getParams().book && getParams().chapter){ YoungsLiteralTranslation.findAllWhere(book: getParams().book.toString(),chapter: getParams().chapter.asType(Integer), [cache: true, readOnly: true]).verseText.each{render it}}
	}
	//readBook
	def readBook() {
		if(getParams().book){ YoungsLiteralTranslation.findAllWhere(book: getParams().book.toString(), [cache: true, readOnly: true]).verseText.each{render it}}
	}
	def read() {
//		Date start = new Date()
		if(getParams().book && getParams().chapter && getParams().verse){ YoungsLiteralTranslation.findAllWhere(book: getParams().book.toString(),chapter: getParams().chapter.asType(Integer),verse: getParams().verse.asType(Integer), [cache: true, readOnly: true]).verseText.each{render it}}
		else if(getParams().book && getParams().chapter && !getParams().verse){ YoungsLiteralTranslation.findAllWhere(book: getParams().book.toString(),chapter: getParams().chapter.asType(Integer), [cache: true], readOnly: true).verseText.each{render it}}
		else if(getParams().book && !getParams().chapter && !getParams().verse){ YoungsLiteralTranslation.findAllWhere(book: getParams().book.toString(), [cache: true], readOnly: true).verseText.each{render it}}
		else if(!getParams().book && !getParams().chapter && !getParams().verse){ YoungsLiteralTranslation.list().verseText.each{render it}}
//		Date stop = new Date()
...
//		render td
	...
	def chapterView() {
		[chapterMapped:YoungsLiteralTranslation.findAllWhere(book: getParams().book.toString(), chapter: getParams().chapter.asType(Integer), [cache: true, readOnly: true])]
	}
		...
		if (bibleMap.isEmpty()) {
			books.each { bookName->
				chapters.get(bookName.toString()).each { chapterNumber->
					bibleMap.put(bookName.toString() + chapterNumber.toString(), YoungsLiteralTranslation.findAllWhere(book: bookName,chapter: chapterNumber, [cache: true, readOnly: true])
					)
				}
			}
		}
	...
	def jsonList() {
		render YoungsLiteralTranslation.list() as JSON
	}
	def xmlList() {
		render YoungsLiteralTranslation.list() as XML
	}
	def searchList() {
		def all = []
		if(getParams().book){
			def thisBook = []
			thisBook = YoungsLiteralTranslation.findAllWhere(book: getParams().book.toString(), [cache: true, readOnly: true])
			all.addAll(thisBook)
		} else {
