...
class BasicEnglishBibleController {
	...
	//readChapter
	def readChapter() {
		if(getParams().book && getParams().chapter){ BasicEnglishBible.findAllWhere(book: getParams().book.toString(),chapter: getParams().chapter.asType(Integer), [cache: true, readOnly: true]).verseText.each{render it}}
	}
	//readBook
	def readBook() {
		if(getParams().book){ BasicEnglishBible.findAllWhere(book: getParams().book.toString(), [cache: true, readOnly: true]).verseText.each{render it}}
	}
	def read() {
//		Date start = new Date()
		if(getParams().book && getParams().chapter && getParams().verse){ BasicEnglishBible.findAllWhere(book: getParams().book.toString(),chapter: getParams().chapter.asType(Integer),verse: getParams().verse.asType(Integer), [cache: true, readOnly: true]).verseText.each{render it}}
		else if(getParams().book && getParams().chapter && !getParams().verse){ BasicEnglishBible.findAllWhere(book: getParams().book.toString(),chapter: getParams().chapter.asType(Integer), [cache: true], readOnly: true).verseText.each{render it}}
		else if(getParams().book && !getParams().chapter && !getParams().verse){ BasicEnglishBible.findAllWhere(book: getParams().book.toString(), [cache: true, readOnly: true]).verseText.each{render it}}
		else if(!getParams().book && !getParams().chapter && !getParams().verse){ BasicEnglishBible.list().verseText.each{render it}}
//		Date stop = new Date()
//		TimeDuration td = TimeCategory.minus( stop, start )
//		render td
	}
	def chapterView() {
		[chapterMapped:BasicEnglishBible.findAllWhere(book: getParams().book.toString(), chapter: getParams().chapter.asType(Integer), [cache: true, readOnly: true])]
	}
		...
		if (bibleMap.isEmpty()) {
			books.each { bookName->
				chapters.get(bookName.toString()).each { chapterNumber->
					bibleMap.put(bookName.toString() + chapterNumber.toString(), BasicEnglishBible.findAllWhere(book: bookName,chapter: chapterNumber, [cache: true, readOnly: true])
					)
				}
			}
		}
	...
	def jsonList() {
		render BasicEnglishBible.list() as JSON
	}
	def xmlList() {
		render BasicEnglishBible.list() as XML
	}
	def searchList() {
		def all = []
		if(getParams().book){
			def thisBook = []
			thisBook = BasicEnglishBible.findAllWhere(book: getParams().book.toString(), [cache: true, readOnly: true])
			all.addAll(thisBook)
		} else {
